 ...
 * @author David Wu (Edits and Supplements)
 */
public class IOUtils {

    public static void readMany(InputReader in, int... vars) {
        Arrays.stream(vars).forEach(value -> value=in.readInt());
    }

    public static void readMany(InputReader in, long... vars) {
        Arrays.stream(vars).forEach(value -> value=in.readLong());
    }

    public static void readMany(InputReader in, BigInteger... vars) {
        Arrays.stream(vars).forEach(value -> value = new BigInteger(in.readString()));
    }

    public static void readMany(InputReader in, char... vars) {
        for (char c : vars) {
            c = in.readCharacter();
        }
    }

    public static void readMany(InputReader in, String... vars) {
        Arrays.stream(vars).forEach(value -> value=in.readString());
    }
